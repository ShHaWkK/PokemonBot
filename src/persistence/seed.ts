...
export function seedIfNeeded() {
  if (!exists("species")) {
    const insert = db.prepare("INSERT INTO species (id, name, types_json, base_stats_json, evolutions_json, mega_json) VALUES (?, ?, ?, ?, ?, ?)");
    const species = [
      { id: 1, name: "Bulbizarre", types: ["Plante","Poison"], base: { hp:45, atk:49, def:49, spa:65, spd:65, spe:45 }, evo: [{ to: 2, level:16 },{ to:3, level:32 }], mega: { stone: "Venusaurite", stats: { hp:45, atk:62, def:63, spa:100, spd:100, spe:40 } } },
      { id: 2, name: "Herbizarre", types: ["Plante","Poison"], base: { hp:60, atk:62, def:63, spa:80, spd:80, spe:60 }, evo: [{ to:3, level:32 }], mega: null },
      { id: 3, name: "Florizarre", types: ["Plante","Poison"], base: { hp:80, atk:82, def:83, spa:100, spd:100, spe:80 }, evo: [], mega: { stone: "Venusaurite", stats: { hp:80, atk:100, def:123, spa:122, spd:120, spe:80 } } },
      { id: 4, name: "Salamèche", types: ["Feu"], base: { hp:39, atk:52, def:43, spa:60, spd:50, spe:65 }, evo: [{ to:5, level:16 },{ to:6, level:36 }], mega: { stone: "Charizardite X", stats: { hp:39, atk:130, def:111, spa:130, spd:85, spe:100 } } },
      { id: 5, name: "Reptincel", types: ["Feu"], base: { hp:58, atk:64, def:58, spa:80, spd:65, spe:80 }, evo: [{ to:6, level:36 }], mega: null },
      { id: 6, name: "Dracaufeu", types: ["Feu","Vol"], base: { hp:78, atk:84, def:78, spa:109, spd:85, spe:100 }, evo: [], mega: { stone: "Charizardite Y", stats: { hp:78, atk:104, def:78, spa:159, spd:115, spe:100 } } },
      { id: 7, name: "Carapuce", types: ["Eau"], base: { hp:44, atk:48, def:65, spa:50, spd:64, spe:43 }, evo: [{ to:8, level:16 },{ to:9, level:36 }], mega: { stone: "Blastoisinite", stats: { hp:44, atk:103, def:120, spa:135, spd:115, spe:78 } } },
      { id: 8, name: "Carabaffe", types: ["Eau"], base: { hp:59, atk:63, def:80, spa:65, spd:80, spe:58 }, evo: [{ to:9, level:36 }], mega: null },
      { id: 9, name: "Tortank", types: ["Eau"], base: { hp:79, atk:83, def:100, spa:85, spd:105, spe:78 }, evo: [], mega: { stone: "Blastoisinite", stats: { hp:79, atk:103, def:120, spa:135, spd:115, spe:78 } } },
      { id: 25, name: "Pikachu", types: ["Electrik"], base: { hp:35, atk:55, def:40, spa:50, spd:50, spe:90 }, evo: [], mega: null },
      { id: 133, name: "Evoli", types: ["Normal"], base: { hp:55, atk:55, def:50, spa:45, spd:65, spe:55 }, evo: [], mega: null },
      { id: 150, name: "Mewtwo", types: ["Psy"], base: { hp:106, atk:110, def:90, spa:154, spd:90, spe:130 }, evo: [], mega: { stone: "Mewtwonite", stats: { hp:106, atk:190, def:130, spa:154, spd:90, spe:150 } } }
    ];
    const toJson = (x: unknown) => JSON.stringify(x);
    const insertMany = db.transaction(() => {
      for (const s of species) {
        insert.run(s.id, s.name, toJson(s.types), toJson(s.base), toJson(s.evo), s.mega ? toJson(s.mega) : null);
      }
    });
    insertMany();
  }
